	...
	async function sendNotification(uids) {
		...
		if (websockets.server) {
			await Promise.all(uids.map(async (uid) => {
				await plugins.hooks.fire('filter:sockets.sendNewNoticationToUid', { uid, notification });
...
Notifications.baseTypes.push('notificationType_teacher-reply');
Notifications.teacherReply = async function (replyData) {
	const teacher = await User.getUser(replyData.teacherUid);
	if (!teacher || teacher.role !== 'teacher') {
		return;
	}
	const question = await getQuestion(replyData.questionId);
	if (!question) {
		return;
	}
	if (teacher.uid === question.uid) {
		return;
	}
	const nid = `notification:teacher-reply:${Date.now()}:${replyData.postId}`;
	const notificationData = {
		nid: nid,
		type: 'notificationType_teacher-reply',
		from: teacher.uid,
		pid: replyData.postId,
		bodyShort: replyData.subject || 'Your teacher has replied to your question!',
		bodyLong: replyData.replyContent,
		path: `/question/${replyData.questionId}`,
		mergeId: `teacher-reply|${replyData.questionId}`,
		importance: 5
	};
	await Notifications.create(notificationData);
	await sendEmail({ uids: [question.uid], notification: notificationData });
	await Notifications.push(notificationData, question.uid);
};
async function getQuestion(questionId) {
	const questionData = await db.getObject(`question:${questionId}`);
	if (!questionData) {
		return null;
	}
	questionData.uid = parseInt(questionData.uid, 10) || 0;
	return questionData;
}
