...
Notifications.markResolved = async function (qid, uid) {
    if (!qid || parseInt(uid, 10) <= 0) {
        return;
    }

    // Fetch question from database
    const question = await db.getObject(`questions:${qid}`);
    if (!question) {
        throw new Error('[[error:no-question-found]]');
    }

    // Mark question as resolved
    await db.setObjectField(`questions:${qid}`, 'resolved', true);

    // Notify students about the resolved status
    const notification = {
        nid: `resolve-${qid}-${Date.now()}`,
        from: uid,
        to: question.students || [],
        content: `Question ${qid} has been marked as resolved.`,
        datetime: Date.now(),
        type: 'resolved',
    };
...
Notifications.notifyReply = async function (replyId, originalPostId, replierUid) {
    ...
    const originalPost = await db.getObject(`posts:${originalPostId}`);
    if (!originalPost) {
        throw new Error('[[error:no-original-post]]');
    }

    const originalPosterUid = originalPost.uid;
    if (!originalPosterUid || originalPosterUid === replierUid) {
        return; // Don't notify if the user replies to their own post
    }

    // Create the notification object
    const notification = {
        nid: `reply-${replyId}-${Date.now()}`,
        from: replierUid,
        to: originalPosterUid,
        content: `You received a reply to your post.`,
        link: `/post/${originalPostId}`, // Adjust based on your URL structure
        datetime: Date.now(),
        type: 'reply',
    };
    ...
    // Save notification
    await db.setObject(`notifications:${notification.nid}`, notification);
    await db.sortedSetAdd(`uid:${originalPosterUid}:notifications:unread`, notification.datetime, notification.nid);

    // Send email if the user has email notifications enabled
    const userSettings = await db.getObject(`user:${originalPosterUid}:settings`);
    if (userSettings?.emailNotifications) {
        await Notifications.sendEmail(originalPosterUid, notification.content, notification.link);
    }
};
