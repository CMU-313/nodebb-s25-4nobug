...
Notifications.create = async function (data) {
    ...
    // Check if this is a student posting a question
    if (data.type === 'notificationType_new-topic' && data.category === 'questions') {
        const instructorUids = await getInstructorUids();
        if (instructorUids.length > 0) {
            const instructorNotification = {
                nid: `instructor-alert-${data.nid}`,
                type: 'notificationType_new-question',
                from: data.from,
                bodyShort: `A new question has been posted: ${data.bodyShort}`,
                path: data.path,
                datetime: now,
            };

            await Notifications.push(instructorNotification, instructorUids);
        }
    }
...
// Helper function to get instructor user IDs
async function getInstructorUids() {
    const instructorGroup = await groups.getMembers('instructors', 0, -1);
